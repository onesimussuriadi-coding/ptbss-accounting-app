...
def render_pembelian_kredit():
    ...
    if f'val_ppn_pembelian_{idx}' not in st.session_state: st.session_state[f'val_ppn_pembelian_{idx}'] = 0.0
    if f'val_pph_pembelian_{idx}' not in st.session_state: st.session_state[f'val_pph_pembelian_{idx}'] = 0.0
    ...
    with st.form(f"form_pembelian_kredit_kompak_{idx}", clear_on_submit=False):
        ...
        with c1:
            ...
            with l1: st.markdown("📌 **No. Bukti / Transaksi**")
            with i1: no_bukti = st.text_input("No Bukti", placeholder="Cth: BKK-CR-2026-001", label_visibility="collapsed", key=f"nobukti_pk_{idx}")
        with c2:
            l2, i2 = st.columns([1.2, 1.8])
            with l2: st.markdown("🏗️ **Business Unit / Proyek**")
            with i2:
                bu_pilihan = st.selectbox("Business Unit", ["-- Pilih Business Unit --"] + list_bu_opt, label_visibility="collapsed", key=f"bu_pk_{idx}")

        # Baris 2: Tanggal Transaksi & Departemen Tujuan
        c3, c4 = st.columns(2)
        with c3:
            l3, i3 = st.columns([1.2, 1.8])
            with l3: st.markdown("📅 **Tanggal Transaksi**")
            with i3: tanggal_trx = st.date_input("Tgl Trx", datetime.now(), label_visibility="collapsed", key=f"tgl_pk_{idx}")
        with c4:
            l4, i4 = st.columns([1.2, 1.8])
            with l4: st.markdown("🎯 **Departemen Tujuan**")
            with i4: dept_tujuan = st.selectbox("Dept", ["Operasional", "HRD", "Logistik", "Maintenance", "HSE", "Akuntansi", "Keuangan"], label_visibility="collapsed", key=f"dept_pk_{idx}")

        # Baris 3: Pemasok / Vendor & No Invoice
        c5, c6 = st.columns(2)
        with c5:
            l5, i5 = st.columns([1.2, 1.8])
            with l5: st.markdown("🏢 **Pemasok / Vendor**")
            with i5: pemasok_pilihan = st.selectbox("Pemasok", ["-- Pilih Pemasok --"] + list_supplier_opt, label_visibility="collapsed", key=f"pemasok_pk_{idx}")
        with c6:
            l6, i6 = st.columns([1.2, 1.8])
            with l6: st.markdown("📄 **No. Invoice Supplier**")
            with i6: no_invoice = st.text_input("Invoice", value="-", label_visibility="collapsed", key=f"inv_pk_{idx}")

        # Input teks opsional jika memilih Pihak Ketiga
        pemasok_final = pemasok_pilihan if pemasok_pilihan not in ["-- Pilih Pemasok --", "-- Ketik Nama Pemasok Lain / Pihak Ketiga --"] else ""
        if pemasok_pilihan == "-- Ketik Nama Pemasok Lain / Pihak Ketiga --":
            pemasok_manual = st.text_input("✍️ Masukkan Nama Pemasok / Pihak Ketiga Lainnya:", placeholder="Ketik nama vendor di sini...", key=f"manual_sup_pk_{idx}")
        ...
        with c7:
            ...
            with l7: st.markdown("⏳ **Jatuh Tempo Hutang**")
            with i7: jatuh_tempo = st.date_input("Jatuh Tempo", datetime.now(), label_visibility="collapsed", key=f"tempo_pk_{idx}")
        with c8:
            l8, i8 = st.columns([1.2, 1.8])
            with l8: st.markdown("📦 **Qty & Satuan**")
            with i8:
                qc, sc = st.columns([1, 1.3])
                with qc: jumlah = st.number_input("Qty", min_value=0.0, value=1.0, step=1.0, label_visibility="collapsed", key=f"qty_pk_{idx}")
                with sc: satuan_pilihan = st.selectbox("Sat", ["Satuan"] + st.session_state.master_satuan, label_visibility="collapsed", key=f"sat_pk_{idx}")
                satuan_final = satuan_pilihan if satuan_pilihan != "Satuan" else "Unit"

        # Baris 5: Alokasi Alat (Dibaca persis dari Kolom Master Alat)
        c9, c10 = st.columns(2)
        with c9:
            l9, i9 = st.columns([1.2, 1.8])
            with l9: st.markdown("🔧 **Alokasi Alat / Unit**")
            with i9: peruntukan_pilihan = st.selectbox("Alat", ["-- Pilih Alokasi Alat --"] + list_alat_opt, label_visibility="collapsed", key=f"alat_pk_{idx}")
            peruntukan_final = peruntukan_pilihan if peruntukan_pilihan != "-- Pilih Alokasi Alat --" else "-"

        st.markdown("---")

        # Uraian / Keterangan Transaksi di Atas Nilai Keuangan
        st.markdown("##### 📝 Uraian / Keterangan Transaksi")
        keterangan = st.text_area("Keterangan", placeholder="Tuliskan keterangan detail pembelian kredit di sini...", height=70, label_visibility="collapsed", key=f"ket_pk_{idx}")
        ...
        with rf1:
            ...
            dpp_val = st.number_input("DPP", min_value=0.0, value=0.0, step=10000.0, format="%.2f", label_visibility="collapsed", key=f"dpp_pk_{idx}")
        with rf2:
            st.markdown("**PPN (Rp)**")
            ppn_val = st.number_input("PPN", min_value=0.0, value=0.0, step=1000.0, format="%.2f", label_visibility="collapsed", key=f"ppn_pk_{idx}")
        with rf3:
            st.markdown("**PPh (Rp)**")
            pph_val = st.number_input("PPh", min_value=0.0, value=0.0, step=1000.0, format="%.2f", label_visibility="collapsed", key=f"pph_pk_{idx}")
        ...
        with b1: submitted = st.form_submit_button("💾 Simpan", use_container_width=True)
        with b2: btn_edit = st.form_submit_button("✏️ Edit", use_container_width=True)
        with b3: btn_tambah = st.form_submit_button("➕ Baris", use_container_width=True)
        with b4: btn_refresh = st.form_submit_button("🔄 Refresh", use_container_width=True)
        ...
        if submitted:
            if not no_bukti or not pemasok_final or not bu_pilihan or bu_pilihan == "-- Pilih Business Unit --":
