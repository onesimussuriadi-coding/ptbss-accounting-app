  ...
  if os.path.exists("master_gudang_bss.xlsx"):
    try:
      ...
      if len(df_gudang.columns) >= 2:
        ...
        list_gudang_opt = (
            df_gudang.iloc[:, 0].dropna().astype(str).str.strip().tolist()
        )
    except Exception:
      pass
  if not list_gudang_opt:
    list_gudang_opt = [
        "GD BBM - Gudang BBM dan Pelumas",
        "GD Part - Gudang Spare Part",
        "GD ATK - Gudang ATK",
    ]
  ...
  with st.container():
    ...
    with c2:
      no_bukti = st.text_input(
          "No Bukti",
          placeholder="Cth: BSS/GDG/VIII/2026/001",
          label_visibility="collapsed",
          key=f"nobukti_gd_{idx}",
      )
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>🏗️ **Business Unit / Proyek Terkait**", unsafe_allow_html=True)
    with c2:
      ...
      with sub_bu2:
        tambah_bu_baru = st.text_input(
            ...
            key=f"tambah_bu_gd_{idx}",
            label_visibility="collapsed",
        )
      bu_final = (
          bu_pilihan if bu_pilihan != "-- Pilih Business Unit --" else ""
      )
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>📏 **Satuan Barang**", unsafe_allow_html=True)
    with c2:
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br><br>📝 **Uraian / Keterangan Material**", unsafe_allow_html=True)
    with c2:
      keterangan = st.text_area(
          "Keterangan",
          placeholder="Nama barang, spesifikasi, atau catatan gudang...",
          label_visibility="collapsed",
          key=f"ket_gd_{idx}",
      )
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>💰 **Estimasi Nilai / Harga (Opsional)**", unsafe_allow_html=True)
    with c2:
      nilai_barang = st.number_input(
          "Estimasi Nilai",
          min_value=0.0,
          step=10000.0,
          format="%.2f",
    ...
    with col_b1:
      if st.button(
          ...
          key=f"btn_save_gd_{idx}",
      ):
        lokasi_gudang_final = (
            lokasi_gudang_pilihan
            if lokasi_gudang_pilihan != "-- Pilih Gudang Sumber / Tujuan --"
            else ""
        )
        if no_bukti and bu_final and lokasi_gudang_final and jumlah > 0:
          ...
          st.error(
              "Mohon lengkapi Nomor Bukti, Lokasi Gudang, Business Unit, dan"
              " Qty > 0."
          )
