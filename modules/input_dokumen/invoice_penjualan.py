...
def render_invoice_penjualan():
  st.markdown(
      "### 🏷️ Modul Khusus: Penerbitan Invoice / Tagihan Penjualan (Piutang"
      " Usaha)"
  )
  ...
  if departemen_tujuan == "-- Pilih Departemen Tujuan --":
    st.warning(
        ...
        " dahulu di atas untuk membuka formulir invoice penjualan."
    )
    ...
    try:
      ...
      if len(df_alat.columns) >= 2:
        ...
        if col_a2 and col_a2 in df_alat.columns:
          list_alat_opt = (
              df_alat[col_a1].astype(str).str.strip()
              + " ("
              + df_alat[col_a2].astype(str).str.strip()
              + ")"
          ).tolist()
        else:
          list_alat_opt = (
  ...
  if f"val_ppn_inv_{idx}" not in st.session_state:
    st.session_state[f"val_ppn_inv_{idx}"] = 0.0
  if f"val_pph_inv_{idx}" not in st.session_state:
    st.session_state[f"val_pph_inv_{idx}"] = 0.0

  with st.container():
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>📅 **Tanggal Invoice**", unsafe_allow_html=True)
    with c2:
      tgl = st.date_input(
          "Tanggal",
          datetime.now(),
    ...
    with c2:
      no_bukti = st.text_input(
          "No Bukti",
          placeholder="Cth: BSS/AR/VIII/2026/001",
          label_visibility="collapsed",
          key=f"nobukti_inv_{idx}",
      )
    ...
    with c1:
      st.markdown(
          "<br>👥 **Pelanggan / Customer (Debitur)**", unsafe_allow_html=True
      )
    with c2:
      ...
      with sub_cust2:
        tambah_pelanggan_baru = st.text_input(
            ...
            key=f"tambah_pelanggan_inv_{idx}",
            label_visibility="collapsed",
        )
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>🏷️ **Nomor Invoice Perusahaan**", unsafe_allow_html=True)
    with c2:
      no_invoice_perusahaan = st.text_input(
          "No Invoice Tagihan",
          placeholder="Cth: INV/BSS/2026/001",
          key=f"inv_perusahaan_inv_{idx}",
          label_visibility="collapsed",
      )

    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>⏳ **Jatuh Tempo Piutang**", unsafe_allow_html=True)
    with c2:
      tgl_jatuh_tempo = st.date_input(
          "Jatuh Tempo Piutang",
          datetime.now(),
          key=f"duedate_ar_inv_{idx}",
          label_visibility="collapsed",
      )
    ...
    with c2:
      ...
      with sub_p1:
        peruntukan_pilihan = st.selectbox(
            ...
            label_visibility="collapsed",
            key=f"peruntukan_sel_inv_{idx}",
        )
      with sub_p2:
        peruntukan_manual = st.text_input(
            "Atau Ketik Bebas",
            placeholder="Ketik manual...",
            label_visibility="collapsed",
            key=f"peruntukan_man_inv_{idx}",
        )
      peruntukan_final = (
          peruntukan_pilihan
      ...
      if peruntukan_manual.strip():
        peruntukan_final = peruntukan_manual.strip()

    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br><br>📝 **Uraian / Keterangan Tagihan**", unsafe_allow_html=True)
    with c2:
      keterangan = st.text_area(
          "Keterangan",
          placeholder="Uraian pekerjaan atau layanan jasa penagihan...",
          label_visibility="collapsed",
          key=f"ket_inv_{idx}",
      )
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>💰 **Nilai Transaksi (DPP / Dasar)**", unsafe_allow_html=True)
    with c2:
      dpp = st.number_input(
          "DPP",
          min_value=0.0,
    ...
    with c2:
      if st.button(
          ...
          key=f"btn_hitung_inv_{idx}",
      ):
        st.session_state[f"val_ppn_inv_{idx}"] = (
            round(dpp * 0.11, 2) if pakai_ppn else 0.0
        )
        st.session_state[f"val_pph_inv_{idx}"] = (
    ...
    c1, c2 = st.columns([1, 2])
    with c1:
      st.markdown("<br>💵 **Total Nilai Piutang / Invoice**", unsafe_allow_html=True)
    with c2:
    ...
    with col_b1:
      if st.button(
          "💾 Simpan Data Invoice Penjualan",
          use_container_width=True,
          key=f"btn_save_inv_{idx}",
      ):
        if (
            dpp > 0
            and no_bukti
            and no_invoice_perusahaan
            and bu_final
            and lawan_transaksi_final
        ):
          data_baru = {
              ...
              "No Invoice": no_invoice_perusahaan,
          ...
          st.error(
              "Mohon lengkapi Nomor Bukti, Pelanggan, No Invoice, Business"
              " Unit, dan nilai DPP > 0."
          )
