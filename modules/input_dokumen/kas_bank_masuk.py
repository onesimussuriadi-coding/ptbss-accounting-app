...
def render_kas_bank_masuk():
    ...
    with st.container():
        ...
        with c1: st.markdown("<br>📅 **Tanggal Penerimaan**", unsafe_allow_html=True)
        with c2: tgl = st.date_input("Tanggal", datetime.now(), label_visibility="collapsed", key=f"tgl_kbm_{idx}")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🧾 **Nomor Bukti / Ref Internal**", unsafe_allow_html=True)
        with c2: no_bukti = st.text_input("No Bukti", placeholder="Cth: BSS/BM/VIII/2026/001", label_visibility="collapsed", key=f"nobukti_kbm_{idx}")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🏦 **Akun Kas / Rekening Bank Penerima**", unsafe_allow_html=True)
        with c2: bank_penerima = st.selectbox("Pilih Kas/Bank", ["-- Pilih Akun Kas / Bank --"] + list_kas_bank_opt, key=f"bank_terima_kbm_{idx}", label_visibility="collapsed")
        ...
        with c2:
            ...
            with sub_cust1:
                pelanggan_pilihan = st.selectbox("Pilih Pelanggan", ["-- Pilih Pelanggan --"] + list_pelanggan_opt, key=f"pihak_ketiga_kbm_{idx}", label_visibility="collapsed")
            with sub_cust2:
                tambah_pelanggan_baru = st.text_input("Pelanggan Baru", placeholder="Ketik baru...", key=f"tambah_pelanggan_kbm_{idx}", label_visibility="collapsed")
        ...
        with c2: jenis_terima = st.radio("Jenis Penerimaan", ["Pelunasan Tagihan (Berdasarkan Invoice)", "Penerimaan Tunai / Non-Invoice (Input Manual)"], key=f"jenis_terima_kbm_{idx}", label_visibility="collapsed")
        ...
        if jenis_terima == "Pelunasan Tagihan (Berdasarkan Invoice)":
            ...
            with c2: 
                ...
                pilih_inv = st.selectbox("Pilih Invoice", ["-- Pilih Invoice --"] + list_inv_opsi, key=f"pilih_inv_kbm_{idx}", label_visibility="collapsed")
                if pilih_inv == "INV-Lainnya (Manual)" or "Belum ada invoice" in pilih_inv:
                    no_invoice_final = st.text_input("Ketik Nomor Invoice Manual", placeholder="Cth: INV/BSS/2026/001", key=f"inv_manual_kbm_{idx}")
                else:
        ...
        with c2:
            ...
            with sub_bu1:
                bu_pilihan = st.selectbox("Business Unit", ["-- Pilih Business Unit --"] + list_bu_opt, label_visibility="collapsed", key=f"bu_kbm_{idx}")
            with sub_bu2:
                tambah_bu_baru = st.text_input("BU Baru", placeholder="Ketik baru...", key=f"tambah_bu_kbm_{idx}", label_visibility="collapsed")
        ...
        with c2: jumlah = st.number_input("Jumlah", min_value=0.0, step=1.0, value=0.0, label_visibility="collapsed", key=f"jml_kbm_{idx}")
        ...
        with c2:
            ...
            with sub_sat1:
                satuan_pilihan = st.selectbox("Pilih Satuan", ["-- Pilih Satuan --"] + st.session_state.master_satuan, label_visibility="collapsed", key=f"satuan_kbm_{idx}")
            with sub_sat2:
                tambah_satuan_baru = st.text_input("Tambah Satuan", placeholder="Baru...", label_visibility="collapsed", key=f"tambah_satuan_kbm_{idx}")
        ...
        with c2:
            peruntukan_pilihan = st.selectbox("Pilih Alat / Unit", ["-- Pilih Alokasi Alat / Unit --"] + list_alat_opt, label_visibility="collapsed", key=f"peruntukan_kbm_{idx}")
        ...
        with col_b1:
            if st.button("💾 Simpan Data Kas Masuk", use_container_width=True, key=f"btn_save_kbm_{idx}"):
                if total_transaksi > 0 and no_bukti and bu_final and lawan_transaksi_final:
                    data_baru = {
                        "Nomor Bukti": no_bukti, "Tanggal": tgl, "Sumber Transaksi": "Kas Bank Masuk (Penerimaan Dana)",
                        ...
                        "DPP": total_transaksi, "PPN": 0.0, "PPH": 0.0, "Total": total_transaksi, 
                    ...
                    st.error("Mohon lengkapi Nomor Bukti, Lawan Transaksi, Business Unit, dan pastikan nominal > 0.")
                    
        with col_b2:
            if st.button("➕ Simpan & Mulai Entri Baru", use_container_width=True, key=f"btn_save_new_kbm_{idx}"):
                if total_transaksi > 0 and no_bukti and bu_final and lawan_transaksi_final:
                    data_baru = {
                        "Nomor Bukti": no_bukti, "Tanggal": tgl, "Sumber Transaksi": "Kas Bank Masuk (Penerimaan Dana)",
                        "Lawan Transaksi": lawan_transaksi_final, "No Invoice": no_invoice_final,
