...
def render_kas_bank_keluar():
    st.markdown("### 📤 Modul Khusus: Kas & Bank Keluar / Pembayaran Tunai")
    ...
    if not list_kas_bank_opt:
        list_kas_bank_opt = ["111.01 - Kas Besar", "111.02 - Kas Kecil", "112.01 - Bank Mandiri"]
    ...
    if f'val_ppn_kbk_{idx}' not in st.session_state: st.session_state[f'val_ppn_kbk_{idx}'] = 0.0
    if f'val_pph_kbk_{idx}' not in st.session_state: st.session_state[f'val_pph_kbk_{idx}'] = 0.0

    with st.container():
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>📅 **Tanggal Pembayaran / Keluar**", unsafe_allow_html=True)
        with c2: tgl = st.date_input("Tanggal", datetime.now(), label_visibility="collapsed", key=f"tgl_kbk_{idx}")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🧾 **Nomor Bukti / Ref Internal**", unsafe_allow_html=True)
        with c2: no_bukti = st.text_input("No Bukti", placeholder="Cth: BSS/BK/VIII/2026/001", label_visibility="collapsed", key=f"nobukti_kbk_{idx}")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🏦 **Akun Kas / Rekening Bank Sumber**", unsafe_allow_html=True)
        with c2: bank_sumber = st.selectbox("Pilih Kas/Bank", ["-- Pilih Akun Kas / Bank Sumber --"] + list_kas_bank_opt, key=f"bank_sumber_kbk_{idx}", label_visibility="collapsed")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🏢 **Penerima Pembayaran / Vendor**", unsafe_allow_html=True)
        with c2:
            sub_sup1, sub_sup2 = st.columns([3, 1])
            with sub_sup1:
                penerima_pilihan = st.selectbox("Pilih Penerima", ["-- Pilih Penerima Pembayaran --"] + list_supplier_opt, key=f"penerima_sel_kbk_{idx}", label_visibility="collapsed")
            with sub_sup2:
                tambah_penerima_baru = st.text_input("Penerima Baru", placeholder="Ketik baru...", key=f"tambah_penerima_kbk_{idx}", label_visibility="collapsed")
            
            lawan_transaksi_final = penerima_pilihan if penerima_pilihan != "-- Pilih Penerima Pembayaran --" else ""
            if tambah_penerima_baru.strip():
                lawan_transaksi_final = tambah_penerima_baru.strip()

        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🏷️ **No. Referensi / Invoice Terkait**", unsafe_allow_html=True)
        with c2: no_invoice_ref = st.text_input("No Referensi", placeholder="Cth: REF/EXPENSE/2026/001", label_visibility="collapsed", key=f"ref_kbk_{idx}")
        ...
        with c2:
            ...
            with sub_bu1:
                bu_pilihan = st.selectbox("Business Unit", ["-- Pilih Business Unit --"] + list_bu_opt, label_visibility="collapsed", key=f"bu_kbk_{idx}")
            with sub_bu2:
                tambah_bu_baru = st.text_input("BU Baru", placeholder="Ketik baru...", key=f"tambah_bu_kbk_{idx}", label_visibility="collapsed")
        ...
        with c2: jumlah = st.number_input("Jumlah", min_value=0.0, step=1.0, value=0.0, label_visibility="collapsed", key=f"jml_kbk_{idx}")
        ...
        with c2:
            ...
            with sub_sat1:
                satuan_pilihan = st.selectbox("Pilih Satuan", ["-- Pilih Satuan --"] + st.session_state.master_satuan, label_visibility="collapsed", key=f"satuan_kbk_{idx}")
            with sub_sat2:
                tambah_satuan_baru = st.text_input("Tambah Satuan", placeholder="Baru...", label_visibility="collapsed", key=f"tambah_satuan_kbk_{idx}")
        ...
        with c2:
            peruntukan_pilihan = st.selectbox("Pilih Alat / Unit", ["-- Pilih Alokasi Alat / Unit --"] + list_alat_opt, label_visibility="collapsed", key=f"peruntukan_kbk_{idx}")
            peruntukan_final = peruntukan_pilihan if peruntukan_pilihan != "-- Pilih Alokasi Alat / Unit --" else "-"

        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br><br>📝 **Uraian / Keterangan Pembayaran**", unsafe_allow_html=True)
        with c2: keterangan = st.text_area("Keterangan", placeholder="Keterangan pengeluaran kas/bank keluar...", label_visibility="collapsed", key=f"ket_kbk_{idx}")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>💰 **Nilai Dasar (DPP)**", unsafe_allow_html=True)
        with c2: dpp = st.number_input("DPP", min_value=0.0, step=10000.0, format="%.2f", value=0.0, label_visibility="collapsed", key=f"val_dpp_kbk_{idx}")
        ...
        with c1: 
            ...
            tarif_pilihan = st.selectbox("Tarif PPh", [0.01, 0.015, 0.0175, 0.02, 0.03, 0.04], format_func=lambda x: f"{x*100}%".replace(".0", ""), key=f"sel_tarif_kbk_{idx}", label_visibility="collapsed")
        with c2:
            if st.button("⚡ KLIK HITUNG PAJAK OTOMATIS", use_container_width=True, key=f"btn_hitung_kbk_{idx}"):
                st.session_state[f'val_ppn_kbk_{idx}'] = round(dpp * 0.11, 2) if pakai_ppn else 0.0
                st.session_state[f'val_pph_kbk_{idx}'] = round(dpp * tarif_pilihan, 2) if pakai_pph else 0.0
                st.success("Kalkulasi PPN & PPh berhasil diterapkan!")
                st.rerun()

        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🔹 **Nominal PPN**", unsafe_allow_html=True)
        with c2: ppn = st.number_input("PPN", min_value=0.0, step=1000.0, format="%.2f", label_visibility="collapsed", key=f"val_ppn_kbk_{idx}")
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>🔹 **Nominal PPh**", unsafe_allow_html=True)
        with c2: pph = st.number_input("PPh", min_value=0.0, step=1000.0, format="%.2f", label_visibility="collapsed", key=f"val_pph_kbk_{idx}")
        
        total_transaksi = (dpp + ppn) - pph
        
        c1, c2 = st.columns([1, 2])
        with c1: st.markdown("<br>💵 **Total Nilai Pengeluaran**", unsafe_allow_html=True)
        with c2: st.markdown(f"### **Rp {total_transaksi:,.2f}**")

        st.divider()
        col_b1, col_b2 = st.columns(2)
        with col_b1:
            if st.button("💾 Simpan Data Kas Keluar", use_container_width=True, key=f"btn_save_kbk_{idx}"):
                if total_transaksi > 0 and no_bukti and bu_final and lawan_transaksi_final:
                    data_baru = {
                        "Nomor Bukti": no_bukti, "Tanggal": tgl, "Sumber Transaksi": "Bank Keluar / Pembayaran",
                        "Lawan Transaksi": lawan_transaksi_final, "No Invoice": no_invoice_ref,
                        "Jatuh Tempo": "-", "Business Unit": bu_final, "Departemen Tujuan": "Keuangan",
                        ...
                        "Status Dokumen": "Menunggu Approval Kabag Keuangan", "Status Jurnal": "Belum Dijurnal"
                    }
                    ...
                    st.error("Mohon lengkapi Nomor Bukti, Penerima, Business Unit, dan pastikan nilai nominal > 0.")
