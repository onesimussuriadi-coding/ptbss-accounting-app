...
if "credentials_dict" not in st.session_state:
  ...
  if os.path.exists(abs_users_path):
    try:
      # Membaca excel tanpa header wajib agar bisa dibaca berdasarkan posisi kolom (A, B, C, D, E)
    ...
    with st.expander("⚙️ Manajemen Akun & Hak Akses", expanded=True):
      with st.form("sidebar_tambah_user"):
        ...
        new_role = st.selectbox(
            ...
            ["Staf", "Kabag", "Keuangan", "Manajer", "Programmer"],
        )
        ...
        if btn_daftarkan:
          if new_username and new_password and new_name:
            if new_username in st.session_state.credentials_dict:
              ...
              save_users_to_excel()
              st.success(
                  f"Akun **{new_username}** berhasil didaftarkan ke Excel!"
              )
              st.rerun()
          else:
...
if menu == "Dashboard Utama":
  ...
  st.info(f"Hak Akses Anda: **{role_kini} {dept_kini}** ({nama_kini})")
  if not st.session_state.data_operasional.empty:
    df_all = st.session_state.data_operasional.copy()
    df_pusat = (
        df_all[df_all["Departemen Tujuan"] == dept_kini]
        if role_kini == "Kabag"
        else df_all
    )
    if not df_pusat.empty:
      ...
      if pilih_bukti != "-- Pilih --":
        ...
        st.write(
            ...
            f"{row.get('Status Dokumen', 'Menunggu Approval Kabag')}**"
        )
        c1, c2, c3 = st.columns(3)
        with c1:
          if role_kini in ["Kabag", "Manajer", "Programmer"] and st.button(
              "✅ Approve Kabag"
          ):
            st.session_state.data_operasional.loc[
                st.session_state.data_operasional["Nomor Bukti"] == pilih_bukti,
                "Status Dokumen",
            ] = f"Disetujui Kabag ➡️ Menunggu Jurnal"
            st.success("Dokumen disetujui!")
            st.rerun()
        with c2:
          if role_kini in ["Keuangan", "Manajer", "Programmer"] and st.button(
              "📝 Verifikasi & Jurnal"
          ):
            st.session_state.data_operasional.loc[
                st.session_state.data_operasional["Nomor Bukti"] == pilih_bukti,
                ["Status Jurnal", "Status Dokumen"],
            ] = ["Sudah Dijurnal", "Menunggu Approval Manajer"]
            st.success("Tercatat & diteruskan ke Manajer!")
            st.rerun()
        with c3:
          if role_kini in ["Manajer", "Programmer"] and st.button(
              "⭐ Approval Final"
          ):
            st.session_state.data_operasional.loc[
                st.session_state.data_operasional["Nomor Bukti"] == pilih_bukti,
            ...
            st.success("Basis persetujuan selesai!")
            st.rerun()
    else:
